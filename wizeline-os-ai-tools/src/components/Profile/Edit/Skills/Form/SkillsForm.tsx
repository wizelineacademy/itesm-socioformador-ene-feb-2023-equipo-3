...
export function SkillItem({ skill }: any) {
  ...
  return (
    <div className="font-inter mr-3 flex items-center justify-between rounded bg-[#e83d44] p-2 font-semibold text-white">
      <span> {skill.name} </span>
      <IconButton
        onClick={() => deleteArtist(skill.id_skills)}
        className="ml-2"
        size="small"
      >
        ...
        <Controller
          ...
          render={({ field }) => (
            <Autocomplete
              ...
              onChange={(e, intermediateSkills) =>
                field.onChange(intermediateSkills)
              }
              value={field.value}
              isOptionEqualToValue={(option, value) =>
                option.value === value.value
